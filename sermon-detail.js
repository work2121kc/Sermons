...
async function fetchSermonDetails(sermonId) {
    ...
    try {
        ...
        const sermon = sermons.find(s => s.id.toString() === sermonId.toString());
        
        if (sermon) {
            renderSermonDetails(sermon);
            // تحديث عنوان الصفحة
            document.title = `${sermon.title} - موقع الخطب الدينية`;
        } else {
            throw new Error('الخطبة غير موجودة');
        }
